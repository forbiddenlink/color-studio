...
if (POSTHOG_KEY) {
  try {
    ...
    console.warn('PostHog analytics failed to initialize:', e)
  }
}
...
let colorHistoryArray = (() => {
  try {
    ...
    console.warn('Failed to load color history from localStorage:', e)
    return []
  }
})()
...
function restoreUndoState(state) {
  ...
  // Restore hex color
  updateInputColor(state.hex)

  // Restore slider values
  slider.value = state.brightness
  sliderText.textContent = `Brightness: ${state.brightness}%`
  saturationSlider.value = state.saturation
  saturationText.textContent = `Saturation: ${state.saturation}%`
  hueSlider.value = state.hue
  hueText.textContent = `Hue: ${state.hue}\u00B0`

  // Restore toggle state
  if (state.isDarken && !toggleBtn.classList.contains('toggled')) {
    toggleBtn.classList.add('toggled')
    lightenText.classList.add('unselected')
    darkenText.classList.remove('unselected')
  } else if (!state.isDarken && toggleBtn.classList.contains('toggled')) {
...
function undo() {
  ...
  undoPointer--
  restoreUndoState(undoStack[undoPointer])
  showToast('Undo')
}
...
function updateUndoRedoButtons() {
  ...
  if (redoBtn) redoBtn.disabled = undoPointer >= undoStack.length - 1
}
...
// Push undo state when sliders are released
slider.addEventListener('change', () => pushUndoState())
saturationSlider.addEventListener('change', () => pushUndoState())
...
function _generateColorRampOklch(startHex, endHex, steps = 5) {
...
function _generateTints(hex, count = 5) {
...
function _generateShades(hex, count = 5) {
  const oklchColor = hexToOklch(hex)
  const shades = []
...
function _alterBrightness(rgb, percentage) {
...
function _alterSaturation(hsl, percentage) {
...
function getAnalogousColorsOklch(oklchColor) {
  const hues = [((oklchColor.h || 0) - 30 + 360) % 360, ((oklchColor.h || 0) + 30) % 360]
  return hues.map((h) => oklchToHex(oklchColor.l, oklchColor.c, h))
}

function getTriadicColorsOklch(oklchColor) {
  const hues = [((oklchColor.h || 0) + 120) % 360, ((oklchColor.h || 0) + 240) % 360]
  return hues.map((h) => oklchToHex(oklchColor.l, oklchColor.c, h))
}

function getSplitComplementaryColorsOklch(oklchColor) {
  const hues = [((oklchColor.h || 0) + 150) % 360, ((oklchColor.h || 0) + 210) % 360]
  return hues.map((h) => oklchToHex(oklchColor.l, oklchColor.c, h))
}
...
function _getTriadicColors(hsl) {
...
function _getSplitComplementaryColors(hsl) {
...
function _getCompoundColors(hsl) {
...
function updateOutputColor() {
  ...
  const brightnessValue = parseInt(slider.value, 10)
  const saturationValue = parseInt(saturationSlider.value, 10)
  const hueValue = parseInt(hueSlider.value, 10)
...
const addToHistory = (hex) => {
  if (!colorHistoryArray.includes(hex)) {
    ...
    try {
      ...
      console.warn('Failed to save color history to localStorage:', e)
    }
    updateColorHistory()
  }
}
...
const updateColorHistory = () => {
  const _historyEmpty = document.getElementById('historyEmpty')
...
clearHistoryBtn.addEventListener('click', () => {
  ...
  try {
    ...
    console.warn('Failed to clear color history from localStorage:', e)
  }
  updateColorHistory()
})
...
function _generateGradientCSSWithFallback() {
  const colors = getGradientColors()
  const colorStops = colors.join(', ')
...
function initGradientGenerator() {
  ...
  angleSlider.addEventListener('input', () => {
    gradientState.angle = parseInt(angleSlider.value, 10)
    if (angleText) {
...
updateOutputColor = function (...args) {
  originalUpdateOutputColor.apply(this, args)
  updateGradientPreview()
}

// Hook gradient update to scheme color changes
const originalDisplayColorScheme = displayColorScheme
displayColorScheme = function (...args) {
  originalDisplayColorScheme.apply(this, args)
  updateGradientPreview()
}
...
function generateShadcnExport() {
  ...
  const _modifiedHsl = rgbToHSL(modifiedRgb.r, modifiedRgb.g, modifiedRgb.b)
  ...
  const accentHex = schemeColors[2] || generateDerivedColor(currentColor.hex, 60, 5, -5)
  ...
  const primaryFgHsl =
    baseHsl.l > 50 ? { h: baseHsl.h, s: baseHsl.s, l: 10 } : { h: baseHsl.h, s: baseHsl.s, l: 98 }
  const secondaryFgHsl =
    secondaryHsl.l > 50
      ? { h: secondaryHsl.h, s: secondaryHsl.s, l: 10 }
      : { h: secondaryHsl.h, s: secondaryHsl.s, l: 98 }
  const accentFgHsl =
    accentHsl.l > 50
      ? { h: accentHsl.h, s: accentHsl.s, l: 10 }
      : { h: accentHsl.h, s: accentHsl.s, l: 98 }
...
function generateDaisyUIExport() {
  const _modifiedHex = rgbToHex(alteredColor.style.backgroundColor)
  ...
  reader.onload = (e) => {
    ...
    img.onload = () => {
      try {
        ...
        showToast('Failed to extract colors from image')
      }
    }
    img.onerror = () => {
...
function applyColorblindTransform(rgb, matrix) {
  ...
  const newR = Math.round(
    Math.min(255, Math.max(0, (matrix[0][0] * r + matrix[0][1] * g + matrix[0][2] * b) * 255))
  )
  const newG = Math.round(
    Math.min(255, Math.max(0, (matrix[1][0] * r + matrix[1][1] * g + matrix[1][2] * b) * 255))
  )
  const newB = Math.round(
    Math.min(255, Math.max(0, (matrix[2][0] * r + matrix[2][1] * g + matrix[2][2] * b) * 255))
  )
...
function updateUIPreviewWithColorblind() {
  ...
  const simModifiedRgb = applyColorblindTransform(
    modifiedRgb,
    colorblindMatrices[currentColorblindMode]
  )
  const simOriginalRgb = applyColorblindTransform(
    originalRgb,
    colorblindMatrices[currentColorblindMode]
  )
  ...
  if (params.has('color')) {
    ...
    if (isValidHex(hex)) {
      ...
      if (params.has('brightness')) {
        const brightness = parseInt(params.get('brightness'), 10)
      ...
      if (params.has('saturation')) {
        const saturation = parseInt(params.get('saturation'), 10)
        if (!Number.isNaN(saturation) && saturation >= -100 && saturation <= 100) {
      ...
      if (params.has('hue')) {
        const hue = parseInt(params.get('hue'), 10)
...
function hookURLUpdates() {
  ...
  const _wrappedUpdateOutputColor = () => {
...
function getSchemeColorsForExport() {
  const swatches = schemeColors.querySelectorAll('.history-color')
  const colors = []
  swatches.forEach((el) => {
  ...
  // Fallback: if no scheme, use base + modified
  if (colors.length === 0) {
    colors.push({ hex: currentColor.hex, name: getColorName(currentColor.hex) })
    const modHex = rgbToHex(alteredColor.style.backgroundColor)
    if (modHex && modHex !== currentColor.hex) {
      colors.push({ hex: modHex, name: getColorName(modHex) })
    }
  }
  return colors
}
...
function exportPaletteAsImage() {
  const colors = getSchemeColorsForExport()
  ...
  canvas.toBlob((blob) => {
    ...
    a.href = url
    a.download = 'color-studio-palette.png'
...
// Initialize Export Image button
document.addEventListener('DOMContentLoaded', () => {
  const exportImageBtn = document.getElementById('exportImage')
  if (exportImageBtn) {
    exportImageBtn.addEventListener('click', exportPaletteAsImage)
  }
})
...
function initContrastChecker() {
  const fgInput = document.getElementById('contrastFg')
  const bgInput = document.getElementById('contrastBg')
  const fgPicker = document.getElementById('contrastFgPicker')
  const bgPicker = document.getElementById('contrastBgPicker')
  const swapBtn = document.getElementById('contrastSwapBtn')
  const suggestBtn = document.getElementById('contrastSuggestBtn')
  const ratioValue = document.getElementById('contrastRatioValue')
  ...
  // Set foreground to current color on init
  const initialFg = currentColor.hex || '#000000'
  fgInput.value = initialFg
  fgPicker.value = initialFg

  function updateContrastDisplay() {
    let fg = fgInput.value.trim()
    let bg = bgInput.value.trim()

    if (!isValidHex(fg) || !isValidHex(bg)) return

    // Normalize short hex
    if (fg.length === 4) fg = `#${fg[1]}${fg[1]}${fg[2]}${fg[2]}${fg[3]}${fg[3]}`
    ...
    function setBadge(el, passes) {
      el.textContent = passes ? 'Pass' : 'Fail'
      el.className = `contrast-badge ${passes ? 'pass' : 'fail'}`
    }

    setBadge(badgeAaNormal, ratio >= 4.5)
    setBadge(badgeAaLarge, ratio >= 3.0)
    setBadge(badgeAaaNormal, ratio >= 7.0)
    setBadge(badgeAaaLarge, ratio >= 4.5)

    preview.style.backgroundColor = bg
    preview.style.color = fg

    fgPicker.value = fg.length === 7 ? fg : fgPicker.value
    bgPicker.value = bg.length === 7 ? bg : bgPicker.value
  }

  fgInput.addEventListener('input', updateContrastDisplay)
  bgInput.addEventListener('input', updateContrastDisplay)
  ...
  suggestBtn.addEventListener('click', () => {
    ...
    if (!isValidHex(bg)) return

    const bgRgb = hexToRGB(bg)
    let fg = fgInput.value.trim()
    if (!isValidHex(fg)) fg = '#000000'
    ...
    for (let l = 0; l <= 1; l += 0.005) {
      const candidateHex = oklchToHex(l, fgOklch.c, fgOklch.h)
      const candidateRgb = hexToRGB(candidateHex)
      const ratio = calculateContrastRatio(candidateRgb, bgRgb)

      if (ratio >= 4.5) {
        const dist = Math.abs(l - fgOklch.l)
        if (dist < bestDist) {
    ...
    if (bestHex) {
      fgInput.value = bestHex
      fgPicker.value = bestHex
      updateContrastDisplay()
      const newRatio = calculateContrastRatio(hexToRGB(bestHex), bgRgb)
      showToast(`Suggested: ${bestHex} (${newRatio.toFixed(2)}:1)`)
    } else {
      const blackRatio = calculateContrastRatio({ r: 0, g: 0, b: 0 }, bgRgb)
      const whiteRatio = calculateContrastRatio({ r: 255, g: 255, b: 255 }, bgRgb)
      const fallback = blackRatio >= whiteRatio ? '#000000' : '#ffffff'
      fgInput.value = fallback
      fgPicker.value = fallback
      updateContrastDisplay()
      showToast(`No match preserving hue, fell back to ${fallback}`)
    }
  })
...
// =============================================================================
// FEATURE: Palette Mockup Previews
// =============================================================================
...
function getPaletteColors() {
  ...
  schemeColorElements.forEach((el) => {
    const hex = el.dataset.originalHex || rgbToHex(el.style.backgroundColor)
    schemeHexes.push(hex)
  })

  const primary = currentColor.hex
  const modified = rgbToHex(alteredColor.style.backgroundColor) || primary
  const secondary = schemeHexes[1] || generateDerivedColor(primary, 30, -10, 5)
  const accent = schemeHexes[2] || generateDerivedColor(primary, 60, 5, -5)
  const bg = generateDerivedColor(primary, 0, -40, 40)

  return { primary, modified, secondary, accent, bg }
}

function updateMockupPreviews() {
  const { primary, modified, secondary, bg } = getPaletteColors()
  const primaryText = getReadableTextColor(primary)
  const secondaryText = getReadableTextColor(secondary)
  const bgText = getReadableTextColor(bg)
  const modifiedText = getReadableTextColor(modified)

  // Dashboard card
  const dashboard = document.getElementById('mockupDashboard')
  if (dashboard) {
    const d = dashboard.querySelector('.mockup-dashboard')
    d.style.backgroundColor = bg
    d.style.color = bgText

    const badge = dashboard.querySelector('.mock-dash-badge')
    badge.style.backgroundColor = secondary
    badge.style.color = secondaryText

    const bars = dashboard.querySelectorAll('.mock-bar')
    bars.forEach((bar, i) => {
      bar.style.backgroundColor = i % 2 === 0 ? primary : secondary
    })

    const icon = dashboard.querySelector('.mock-dash-icon')
    icon.style.color = secondary
  }

  // Landing hero
  const hero = document.getElementById('mockupHero')
  if (hero) {
    const heroBg = hero.querySelector('.mock-hero-bg')
    const primaryRgb = hexToRGB(primary)
    const secondaryRgb = hexToRGB(secondary)
    heroBg.style.background = `linear-gradient(135deg, rgba(${primaryRgb.r},${primaryRgb.g},${primaryRgb.b},0.9), rgba(${secondaryRgb.r},${secondaryRgb.g},${secondaryRgb.b},0.9))`

    const heroContent = hero.querySelector('.mock-hero-content')
    heroContent.style.color = primaryText

    const cta = hero.querySelector('.mock-hero-cta')
    cta.style.backgroundColor = modified
    cta.style.color = modifiedText

    const tagline = hero.querySelector('.mock-hero-tagline')
    tagline.style.color = primaryText
  }

  // Mobile app
  const mobile = document.getElementById('mockupMobile')
  if (mobile) {
    const status = mobile.querySelector('.mock-phone-status')
    status.style.backgroundColor = primary
    status.style.color = primaryText

    const nav = mobile.querySelector('.mock-phone-nav')
    nav.style.backgroundColor = primary
    nav.style.color = primaryText

    const body = mobile.querySelector('.mock-phone-body')
    body.style.backgroundColor = bg
    body.style.color = bgText

    const cards = mobile.querySelectorAll('.mock-phone-card')
    cards.forEach((card) => {
      card.style.backgroundColor = generateDerivedColor(bg, 0, -5, -5)
    })

    const dots = mobile.querySelectorAll('.mock-phone-dot')
    dots.forEach((dot) => {
      dot.style.backgroundColor = secondary
    })

    const progressFill = mobile.querySelector('.mock-phone-progress-fill')
    if (progressFill) {
      progressFill.style.backgroundColor = primary
    }

    const tabBar = mobile.querySelector('.mock-phone-tab-bar')
    tabBar.style.backgroundColor = bg
    const bgTextRgb = hexToRGB(bgText)
    tabBar.style.borderTopColor = `rgba(${bgTextRgb.r},${bgTextRgb.g},${bgTextRgb.b},0.1)`

    const tabs = mobile.querySelectorAll('.mock-tab')
    tabs.forEach((tab) => {
      tab.style.color = tab.classList.contains('active') ? primary : bgText
    })
  }
}

// =============================================================================
// FEATURE: Color Harmony Scoring
// =============================================================================

function getHarmonyPaletteHues() {
  const schemeColorElements = document.querySelectorAll('#schemeColors .history-color')
  const hues = []
  schemeColorElements.forEach((el) => {
    const hex = el.dataset.originalHex || rgbToHex(el.style.backgroundColor)
  ...
  if (hues.length === 0) {
    const modHex = rgbToHex(alteredColor.style.backgroundColor) || currentColor.hex
    hues.push({ hex: currentColor.hex, hsl: currentColor.hsl })
    const modRgb = hexToRGB(modHex)
    hues.push({ hex: modHex, hsl: rgbToHSL(modRgb.r, modRgb.g, modRgb.b) })
  }
  return hues
}

function computeContrastScore(palette) {
  if (palette.length < 2) return { score: 0, avgRatio: 1 }
  let totalRatio = 0
  let pairs = 0
  for (let i = 0; i < palette.length - 1; i++) {
    const rgb1 = hexToRGB(palette[i].hex)
    const rgb2 = hexToRGB(palette[i + 1].hex)
    totalRatio += calculateContrastRatio(rgb1, rgb2)
    pairs++
  }
  const avgRatio = totalRatio / pairs
  const score = Math.min(100, Math.round((Math.log(avgRatio) / Math.log(21)) * 100))
  return { score, avgRatio }
}

function detectHarmonyType(palette) {
  if (palette.length < 2) return { type: 'None', score: 0 }

  const hues = palette.map((p) => p.hsl.h)
  const diffs = hues.slice(1).map((h) => {
    let d = Math.abs(h - hues[0])
    if (d > 180) d = 360 - d
    return d
  })
  ...
  if (palette.length === 2) {
    if (diffs.some((d) => Math.abs(d - 180) < tolerance)) {
      type = 'Complementary'
      score = 90
    } else if (diffs.some((d) => d < 40)) {
      type = 'Analogous'
      score = 80
    } else {
      type = 'Custom'
    ...
    const isTriadic = diffs.every(
      (d) => Math.abs(d - 120) < tolerance || Math.abs(d - 240) < tolerance
    )
    const isAnalogous = diffs.every((d) => d < 60)
    const isSplitComp =
      diffs.some((d) => Math.abs(d - 150) < tolerance) &&
      diffs.some((d) => Math.abs(d - 30) < tolerance || Math.abs(d - 150) < tolerance)
    const hasComplement = diffs.some((d) => Math.abs(d - 180) < tolerance)
...
function computeAccessibilityScore(palette) {
  if (palette.length < 2) return { score: 0, passCount: 0, totalPairs: 0 }
  ...
  for (let i = 0; i < palette.length; i++) {
    for (let j = i + 1; j < palette.length; j++) {
      const rgb1 = hexToRGB(palette[i].hex)
      const rgb2 = hexToRGB(palette[j].hex)
      const ratio = calculateContrastRatio(rgb1, rgb2)
      totalPairs++
      if (ratio >= 4.5) passCount++
    }
  ...
  const score = totalPairs > 0 ? Math.round((passCount / totalPairs) * 100) : 0
  return { score, passCount, totalPairs }
}

function setGaugeArc(arcId, percent) {
...
function updateHarmonyScoring() {
  const palette = getHarmonyPaletteHues()

  const contrastResult = computeContrastScore(palette)
  const contrastScore = contrastResult.score || 0
  const contrastValueEl = document.getElementById('contrastValue')
  const contrastDetailEl = document.getElementById('contrastDetail')
  if (contrastValueEl) contrastValueEl.textContent = contrastScore
  if (contrastDetailEl)
    contrastDetailEl.textContent = `Avg ${(contrastResult.avgRatio || 1).toFixed(1)}:1`
  setGaugeArc('contrastArc', contrastScore)

  const harmonyResult = detectHarmonyType(palette)
  const harmonyValueEl = document.getElementById('harmonyValue')
  const harmonyDetailEl = document.getElementById('harmonyDetail')
  if (harmonyValueEl) harmonyValueEl.textContent = harmonyResult.score
  if (harmonyDetailEl) harmonyDetailEl.textContent = harmonyResult.type
  setGaugeArc('harmonyArc', harmonyResult.score)

  const accessResult = computeAccessibilityScore(palette)
  const accessValueEl = document.getElementById('accessValue')
  const accessDetailEl = document.getElementById('accessDetail')
  if (accessValueEl) accessValueEl.textContent = accessResult.score
  if (accessDetailEl)
    accessDetailEl.textContent = `${accessResult.passCount}/${accessResult.totalPairs} AA`
  setGaugeArc('accessArc', accessResult.score)

  const overall = Math.round(
    contrastScore * 0.35 + harmonyResult.score * 0.3 + accessResult.score * 0.35
  )
  const overallValueEl = document.getElementById('overallValue')
  const overallDetailEl = document.getElementById('overallDetail')
  if (overallValueEl) overallValueEl.textContent = overall
  if (overallDetailEl) {
    if (overall >= 80) overallDetailEl.textContent = 'Excellent'
    else if (overall >= 60) overallDetailEl.textContent = 'Good'
    else if (overall >= 40) overallDetailEl.textContent = 'Fair'
    else overallDetailEl.textContent = 'Needs work'
  }
  setGaugeArc('overallArc', overall)
}

// Observe scheme changes to update mockups and harmony
const schemeObserver = new MutationObserver(() => {
  updateMockupPreviews()
  updateHarmonyScoring()
})
if (schemeColors) {
  schemeObserver.observe(schemeColors, { childList: true })
}

// Observe altered color style changes for real-time updates
const alteredStyleObserver = new MutationObserver(() => {
  updateMockupPreviews()
  updateHarmonyScoring()
})
if (alteredColor) {
  alteredStyleObserver.observe(alteredColor, { attributes: true, attributeFilter: ['style'] })
}
